...
const filteredAdmins = computed(() => {
  ...
  return props.admins.filter(user => 
...
const editAdmin = (admin) => {
  ...
  editForm.dob = admin.dob ? admin.dob.split('T')[0] : '' // Format date for input
  editForm.education_qualification = admin.education_qualification
  editForm.institute = admin.institute
  editForm.password = ''
...
const updateAdmin = () => {
  ...
  editForm.put(`/admin/users/admins/${editingAdmin.value.id}`, {
    preserveScroll: true,
    onSuccess: () => {
      closeEditModal()
      editForm.reset()
...
const deleteAdmin = (admin) => {
  if (confirm(`Are you sure you want to delete ${admin.name}? This action cannot be undone.`)) {
    router.delete(`/admin/users/admins/${admin.id}`, {
      preserveScroll: true,
    })
  }
}
