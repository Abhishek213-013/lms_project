      ...
      <div class="container">
        <div class="courses-layout">
              ...
              <div class="student-info">
                <div class="student-avatar">
                  <img v-if="courses.user.avatar" 
                       ...
                       :alt="courses.user.name + ' ' + t('Avatar')" 
                       class="avatar-image">
                  <i v-else class="fas fa-user-circle"></i>
                </div>
                <div class="student-details">
                  <div class="student-name">{{ courses.user.name }}</div>
                  <div class="student-email">{{ courses.user.email }}</div>
                  <div class="student-roll" v-if="courses.user.student_info">
          ...
          <div class="courses-main-content">
            ...
            <!-- Enrolled Courses -->
            <div v-else-if="activeTab === 'enrolled' && courses.enrolled.length > 0" class="tab-content">
              <div class="courses-grid">
                <div v-for="course in courses.enrolled" :key="course.id" class="course-card">
                  <div class="course-image">
                    <img v-if="course.thumbnail" :src="course.thumbnail" :alt="course.title" class="course-thumbnail">
                  ...
                  <div class="course-content">
                    ...
                    <div class="course-actions">
                      <button class="btn-continue" @click="continueCourse(course)" :disabled="continuingCourse === course.id">
                        <i class="fas fa-play"></i>
                        {{ course.progress === 0 ? t('Start Learning') : t('Continue') }}
                        <span v-if="continuingCourse === course.id" class="loading-dots"></span>
                      </button>
            ...
            <!-- Completed Courses -->
            <div v-else-if="activeTab === 'completed' && courses.completed.length > 0" class="tab-content">
              <div class="courses-grid">
            ...
            <!-- Wishlist -->
            <div v-else-if="activeTab === 'wishlist' && courses.wishlist.length > 0" class="tab-content">
              <div class="courses-grid">
                <div v-for="course in courses.wishlist" :key="course.id" class="course-card wishlist">
                  <div class="course-image">
                    ...
                    <div class="wishlist-actions">
                      <button class="btn-remove-wishlist" @click="removeFromWishlist(course)" :disabled="removingWishlist === course.id">
                        <i class="fas fa-heart"></i>
                        <span v-if="removingWishlist === course.id" class="loading-dots"></span>
                      </button>
                    </div>
                  </div>
                  ...
                  <div class="course-content">
                    ...
                    <div class="course-price-section">
                      ...
                      <button class="btn-enroll" @click="enrollCourse(course)" :disabled="enrollingCourse === course.id">
                        {{ t('Enroll Now') }}
                        <span v-if="enrollingCourse === course.id" class="loading-dots"></span>
                      </button>
                    </div>
                  </div>
                </div>
...
const continueCourse = async (course) => {
  continuingCourse.value = course.id
  try {
    const response = await fetch(`/api/my-courses/${course.id}/continue`, {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      ...
      alert(t('Failed to continue course: {error}', { error: result.message || 'Unknown error' }))
    }
  } catch (error) {
    alert(t('Error continuing course: {error}', { error: error.message }))
  } finally {
    continuingCourse.value = null
  }
}
...
const downloadCertificate = (course) => {
  alert(t('Downloading certificate for: {course}', { course: course.title }))
  // Implement certificate download logic
}

const reviewCourse = (course) => {
  router.get(`/course/${course.id}/review`)
}
...
const removeFromWishlist = async (course) => {
  ...
  removingWishlist.value = course.id
  try {
    const response = await fetch(`/api/my-courses/${course.id}/remove-wishlist`, {
      method: 'DELETE',
      headers: {
        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
      }
...
const enrollCourse = async (course) => {
  enrollingCourse.value = course.id
  try {
    const response = await fetch(`/api/my-courses/${course.id}/enroll`, {
      method: 'POST',
      headers: {
        'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
        'Content-Type': 'application/json'
