    ...
    <div class="flex-1 ml-64">
      ...
      <div class="p-6">
        ...
        <div class="flex justify-between items-center mb-6">
          <div>
            <h1 class="text-2xl font-bold text-gray-900">Class {{ $route.params.grade }} - Subjects</h1>
          ...
          <div class="flex space-x-3">
            <button class="text-blue-600 hover:text-blue-800 text-sm font-medium">
              <router-link to="/admin/courses/all-courses">← Back to Classes</router-link>
            </button>
            <button 
              @click="refreshData"
...
const fetchSubjects = async (grade) => {
  try {
    ...
    const response = await apiClient.get(`/classes/${grade}/subjects`)
...
const refreshData = () => {
  ...
  fetchSubjects(route.params.grade)
}

const viewSubjectTeachers = (subjectId) => {
  console.log(`🎯 Navigating to subject ${subjectId} teachers for grade ${route.params.grade}`)
  router.push(`/admin/courses/class/${route.params.grade}/subject/${subjectId}/teachers`)
}
...
const handleStorageChange = (event) => {
  if (event.key === 'teacherAssignmentUpdate' && event.newValue) {
    try {
      ...
      if (data.grade && data.grade.toString() === route.params.grade) {
...
const handleTeacherAssigned = (event) => {
  ...
  if (event.detail.grade && event.detail.grade.toString() === route.params.grade) {
...
onMounted(() => {
  console.log('🚀 ClassSubjects component mounted for grade:', route.params.grade)
