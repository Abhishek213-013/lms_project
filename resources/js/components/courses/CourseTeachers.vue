    ...
    <div class="flex-1 ml-64">
      ...
      <div class="p-6">
        ...
        <div class="mt-6 flex justify-start">
          <router-link 
            :to="`/admin/courses/course/${route.params.courseId}/details`" 
...
const fetchCourseTeachers = async () => {
  try {
    ...
    console.log('📡 Fetching course teachers data for course ID:', route.params.courseId)
    
    // Get course details and assigned teachers
    const courseResponse = await apiClient.get(`/courses/${route.params.courseId}/teachers`)
...
const fetchTeachersAlternative = async () => {
  try {
    ...
    // Get course details
    const courseResponse = await apiClient.get(`/courses/${route.params.courseId}`)
    if (courseResponse.data.success) {
...
const assignTeacher = async (teacherId) => {
  try {
    ...
    console.log('📡 Assigning teacher:', teacherId, 'to course:', route.params.courseId)
    
    const response = await apiClient.post(`/courses/${route.params.courseId}/teachers`, {
...
const removeTeacher = async (teacherId) => {
  try {
    ...
    console.log('📡 Removing teacher:', teacherId, 'from course:', route.params.courseId)
    
    const response = await apiClient.delete(`/courses/${route.params.courseId}/teachers/${teacherId}`)
