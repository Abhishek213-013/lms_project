    ...
    <div class="flex-1 ml-64">
      ...
      <div class="p-6">
        ...
        <div class="flex justify-between items-center mb-6">
          <div>
            <h1 class="text-2xl font-bold text-gray-900">Class {{ $route.params.grade }} - Enrollments</h1>
              ...
              <div class="mb-6">
                ...
                <div v-if="subject.teacher" class="flex items-center justify-between p-4 bg-blue-50 rounded-lg border border-blue-100">
                  ...
                  <div class="text-right">
                    ...
                    <router-link 
                      :to="`/admin/courses/class/${$route.params.grade}/subject/${subject.id}/teachers`"
...
const totalStudents = computed(() => {
  return enrollmentData.value.subjects.reduce((total, subject) => total + subject.students.length, 0)
})
...
const fetchClassEnrollments = async (grade) => {
  try {
    loading.value = true
    error.value = ''
    dataSource.value = 'database'
    
    console.log('📡 Fetching class enrollments for grade:', grade)
    
    // Try the new endpoint first
    const response = await apiClient.get(`/courses/classes/${grade}/enrollments`)
    ...
    if (response.data.success) {
      ...
      error.value = response.data.message || 'Failed to fetch enrollment details'
      dataSource.value = 'mock'
      // Fallback to mock data
      enrollmentData.value = getMockEnrollments(grade)
    }
  } catch (err) {
...
const getMockEnrollments = (grade) => {
  ...
  return {
    ...
    subjects: subjects.map((subject, index) => ({
      ...
      students: index % 2 === 0 ? students.slice(0, 3) : students.slice(0, 5),
...
onMounted(() => {
  ...
  checkAuthentication()
  fetchClassEnrollments(route.params.grade)
})

watch(() => route.params.grade, (newGrade) => {
  if (newGrade) {
    fetchClassEnrollments(newGrade)
  }
})
</script>