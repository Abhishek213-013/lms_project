...
const totalStudents = computed(() => {
  return classes.value.reduce((total, classItem) => total + classItem.studentCount, 0)
})

const averageClassSize = computed(() => {
  return classes.value.length > 0 ? Math.round(totalStudents.value / classes.value.length) : 0
})

const capacityPercentage = computed(() => {
  return Math.round((totalStudents.value / (classes.value.length * 40)) * 100)
})
...
const fetchClasses = async () => {
  try {
    ...
    // Use the correct endpoint for classes
    ...
    if (response.data.success) {
      ...
      error.value = response.data.message || 'Failed to fetch enrollment data'
...
const getMockClasses = () => {
  return Array.from({ length: 12 }, (_, i) => ({
    ...
    grade: i + 1,
    name: `Class ${i + 1}`,
    studentCount: Math.floor(Math.random() * 35) + 5, // 5-40 students
    subjectCount: 6, // Standard 6 subjects
    capacity: 40
  }))
}
...
const viewClassEnrollments = (grade) => {
  ...
  router.push(`/admin/courses/enrollments/class/${grade}`)
}
